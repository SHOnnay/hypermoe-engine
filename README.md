...
memory management across VRAM, pinned RAM, ordinary RAM, and NVMe. Phase 3 adds a
hardware abstraction with an always-available CPU backend and an optional CUDA
backend, page-locked staging buffers, stream/event-driven transfers, runtime
hardware detection, and measured hardware benchmarks. There is intentionally no
transformer inference, model adapter, or custom CUDA kernel implementation yet.
